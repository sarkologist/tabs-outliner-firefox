...
- Verification: `pnpm test` and `pnpm run build` passed. A fresh in-browser `tabsOutlinerProfile` after this change is still needed to measure Firefox `storage.local.set` directly, but the synthetic storage payload and stringify path now show bounded steady-state writes for small structural mutations.
