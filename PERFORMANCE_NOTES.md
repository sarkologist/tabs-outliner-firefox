...
| Native window focus-gain (`windows.onFocusChanged`, real window) | `O(w)` window-shell corroboration (`windows.getAll`, `populate:false` — no `tabs.query`) + `O(1)` in-place active-flag flip via the warm index; full reconcile only on the ambiguous fallback | `O(w)` to confirm the focused window + `O(1)` flip | At optimum for the common case. Forfeits no absence proof (focus-gain was already `partial` / `closeMissing:false`, so it never closed missing resources). Falls back to the full `O(w log w + n)` reconcile when the focused window is unknown (`found:false`), focus truth is ambiguous (not exactly one focused normal window), the outline has drifted to >1 active window (the full reconcile re-derives all window active flags), or the shell read fails. |
| Metadata-change corroboration re-query (`tabs.onUpdated` title/url/favicon) | `O(tabs in changed windows)` — two scoped `tabs.query({ windowId })` reads over only the distinct windows of the changed tabs (2026-06-19); global fallback only when a changed tab has no resolvable window id | `O(changed tabs)` to refetch each changed row | Near optimum: scopes the re-query to the affected window(s) instead of every window. A per-tab `tabs.get` would reach `O(changed tabs)` but adds a new api dependency and reject-handling; the window scope captures the dominant all-windows→one-window win. The two-read stale-defeat is retained. |
| Snapshot corroboration re-query (full `closeMissing` refresh / activation, in `corroborateMissingOrMismatchedLiveTabs`) | when every symptom is window-local (`missingTabCount == 0 && mismatchedTabCount == 0`): `O(tabs in the suspicious/order windows)` — one scoped `tabs.query({ windowId })` per affected window, overlaid on the first raw snapshot (2026-06-21); otherwise the original second global `getNormalWindows` | `O(tabs in the suspicious windows)` to re-read each suspicious window | Near optimum for the window-local case (the activation/refresh convoy: the second global read was `O(total live tabs)`). `missing`/`mismatched` keep the global re-read on purpose — a vanished/relocated tab can only be resolved against the whole-browser view (proving absence). The first base snapshot is still one global read. |
| Full runtime reconciliation fallback | `O(w log w + n)` plus browser snapshot cost, then `O(n)` diff or full-state fallback | `O(w + n)` if full validation is required; effectively `O(0)` when avoided | This is the correctness fallback. Main win is preventing narrow events from entering it (now including native focus-gain, alongside the focus-loss early-return); secondary win is trimming avoidable sort/diff work inside it. |
...
### 2026-06-21: Scope the snapshot corroboration re-query to the suspicious window(s)

A full `closeMissing` refresh (a tab activation, or a `refresh` message) takes one global `getNormalWindows` snapshot — `windows.getAll` + a global `tabs.query({})` whose cost scales with **total** live tabs across every window — then, when the snapshot disagrees with state, re-reads a **second** global `getNormalWindows` to corroborate (defeat a stale first read) in `corroborateMissingOrMismatchedLiveTabs`. On a churny multi-window session those two global tab queries run back-to-back, and the browser is often busy with the very tab operation that triggered the refresh, so each can spike to seconds.

In `dist/tabs-outliner-profile-2026-06-21(2).json` this is the worst convoy in the capture: a tab activation (tab 1594) → `getWindows` **3832ms** + `getWindows.corroborate` **5434ms** = a **9421ms** `refreshFromRuntime` mutation; because the mutation scheduler is serial and non-preemptible, that low-priority refresh then head-of-line-blocked the user's tab-close (`waited 3308ms`) and `restoreNode` (`waited 382ms`), for a ~10.5s frozen UI. The corroboration query carried `suspiciousShapeTabCount: 2, missingTabCount: 0, mismatchedTabCount: 0` — all the symptoms were window-local.

Fix: when **every** detected symptom is window-local (suspicious-shape tabs present in their known window, and order-mismatched windows — `missingTabCount == 0 && mismatchedTabCount == 0`), corroborate by re-reading only those windows (`getWindowTabsByIds`, one scoped `tabs.query({ windowId })` each) and overlaying the fresh tabs onto the first **raw** snapshot. The overlay keeps the result globally complete, so the downstream `closeMissing` reconcile never mistakes an un-re-queried window for an emptied one. `missing`/`mismatched` stay on the global re-read: a tab that vanished from, or relocated across, windows can only be resolved by the whole-browser view — the "narrow only events that can't require proving absence" rule. The raw overlay base is withheld (forcing the global path) when the earlier no-event order corroboration already swapped in a different fresh snapshot, so the base always corresponds to the snapshot being corroborated.

Relocation-race guard (caught by the codex-cli review and reproduced as a test): if a suspicious tab moves to **another** window in the gap between the first snapshot and the scoped re-query, the scoped re-read of its old window shows it gone while the destination window is still frozen at the first read — the overlay would make it look missing and a `closeMissing` reconcile would close a tab that actually relocated. Guard: the scoped re-read must preserve each window's tab-id **membership** (only shape/order may differ, via `sameNumberSet`); any window that gained or lost a tab id falls the whole corroboration back to the global `getNormalWindows`. Membership equality provably introduces no new missing/mismatched tab (both were 0), so `closeMissing` stays exactly as safe as the un-scoped path.

- **Asymptotics:** suspicious/order snapshot corroboration `O(total live tabs)` second global read → `O(tabs in the suspicious windows)` scoped read (`Current Asymptotics Audit` row added). The first base snapshot and the `missing`/`mismatched` corroboration are unchanged. No save-shape/timing change.
- **Tests:** new `runtime-snapshot.test.ts` unit tests for `getWindowTabsByIds` (scoped, de-duplicated, incognito-filtered). New `controller.test.ts` tests: (a) red→green — a quiet URL change on tab 1 (window 10 of 3) refreshes with exactly **one** global query (the base) and a corroboration scoped to `{ windowId: 10 }`, never windows 20/30 (red on baseline: 2 global queries); (b) a **missing** tab still corroborates globally (≥2 global queries, no scoped read), so absence-proof is not narrowed; (c) the relocation-race guard — a suspicious tab that moves to window 30 between the two reads falls back to the global read and is **relocated, not closed** (verified red→green: with the membership check disabled it does only one global read and the tab is mis-closed).
- **Safety:** full suite green (861/2) incl. `controller.test.ts` (305) lifecycle/adversarial/runtime-trace generated traces, `runtime-reconciler.test.ts` (41), and the reconciliation perf profile test. Independent `codex-cli` adversarial review run over the branch diff — it found the relocation race (fixed + regression-tested as above); no other findings. `node scripts/perf-runtime-guard.mjs --hard-only` PASS (9 scenarios; `tabsQueryMs`/`windowsGetAllMs`/save/journal/broadcast counters identical to baseline). The only timing overage is the chronic, pre-existing `command-group-live-leaf firstBroadcastMs` (bounced 153/154/169/171 across runs; sibling `command-move-leaf` passes; `main` misses it identically; report-only under `--hard-only`) — a grouping command never enters the refresh-corroboration path this edits. Not a save-timing/shape or sidebar-projection change, so those lanes do not apply.
...
A `tabs.onUpdated` carrying a title/url/favicon change runs `corroborateMetadataEventEvidence` before trusting the (possibly event-local stale) change: it re-queries the browser twice and reads back the changed tab's fresh row, discarding the first read to defeat Firefox's stale-first-query-inside-the-handler quirk. Both reads were global `api.tabs.query({})` over **every** window, even though the function only ever reads back each changed tab's own row (`currentTabsById.get(evidence.tab.id)`). On the user's multi-window session that global scan is `O(total live tabs)` and, worse, runs *while Firefox is busy processing the same operation* — a feedback loop. In `dist/tabs-outliner-profile-2026-06-19.json` the two corroboration queries cost **137ms + 254ms = 391ms** for a single-tab metadata echo (vs 2-3ms when the browser was idle), the single largest avoidable chunk of an event-echo reconcile.

The two reads now re-query only the distinct windows of the tabs that actually need corroboration (`api.tabs.query({ windowId })` per changed window, usually one), falling back to the global query only if a changed tab has no resolvable window id. Semantics are unchanged: the function still reads back each changed tab by id, and a tab that has since moved windows simply won't appear in its old window's result — exactly the existing "tab moved → keep original evidence" branch. The two-read stale-defeat is preserved. This is independent of, and complementary to, the broader echo-absorption work (a genuine *external* metadata update — e.g. a background tab's favicon resolving — still corroborates, now cheaply); absorbing command-owned echoes removes the reconcile entirely for the self-induced case.

- **Asymptotics:** metadata corroboration re-query `O(total live tabs)` → `O(tabs in changed windows)`; no save-shape or transport change (it only changes which tabs are queried). Trace detail now carries `windowCount` (`"all"` for the fallback).
- **Tests:** new deterministic `controller.test.ts` test — a title/favicon `onUpdated` across a 3-window session corroborates via `tabs.query({ windowId: 10 })` and never a global `tabs.query({})`, while the metadata still lands on the node. Full controller suite green, including the oracle-backed `mh-*` metadata/stale-query/reordered-query regression traces and the window-order corroboration tests (which exercise the separate `getNormalWindows` snapshot, untouched here).
- **Guards:** `pnpm perf:runtime-guard` hard counters green (`--hard-only` PASS). The one timing overage (`command-group-live-leaf` firstBroadcastMs ~175 > 138) is **pre-existing and machine-bound** — `main` without this change also reports ~163 on this machine, and a group command cannot reach the corroboration path. Not a sidebar-path or save-timing change, so the projection and storage-fault lanes do not apply.

### 2026-06-18: Narrow native window focus-gain to a corroborated in-place active-flag flip

A native window focus-gain (`windows.onFocusChanged` for a real window) previously queued a full runtime refresh: `getNormalWindows` — whose cost on a large session is dominated by its global `tabs.query({})` over every tab (the environment-bound up-to-~2.5s call the user reported) — followed by a whole-tree clone + `reconcileWithWindows` over every window + material diff, even though the only material delta of a pure focus switch is the window `active` flag. `runtimeSnapshotMateriallyMatchesState` treats `windowNode.active !== windowInfo.focused` as material, so *every* window switch entered that full fallback (~187-779ms of controller work on top of the query). The clone + diff is `O(total outline nodes)` — and for this user the outline is ~25k nodes that are ~99.8% **closed** (saved history), so the whole-tree work is paid on every focus switch regardless of how few tabs are actually open.

A pure focus-gain refresh now takes a narrow path (`tryNativeFocusGainInPlaceForRefresh` in `controller.ts`): corroborate against fresh window truth with `getNormalWindowShells` (`windows.getAll`, `populate:false` — window focused-flags only, **no** `tabs.query`) and flip the active flag with the existing `focusRuntimeWindowInPlace` helper (the same one the command-owned focus echo uses). It is reached only for a pure focus-gain (no event tabs, no coalesced activation, `closeMissing:false`, not a forced snapshot, ≥1 focused window queued).

Crucially it is **corroborated, not event-trusting**: a native focus event can be stale (a command can re-focus a different window between the event firing and the deferred refresh running — the `concurrent-focused-window-then-group` generated traces), so the flip targets whichever window the fresh shells report focused, not the event's id. Anything ambiguous — focused window unknown (`found:false`), not exactly one focused normal window, or the shell read fails — falls back to the full reconcile, which discovers/attaches as before ("keep fallbacks").

Safety: focus-gain already used `partial` snapshot confidence with `closeMissing:false`, so it never proved absence — narrowing forfeits no close/delete detection (a partial snapshot could not close a missing resource anyway), and every tab-shape change keeps its own event path. The flip mutates only the `active` flag of known live window nodes (I-9 live refs intact; I-11/I-12 runtime scopes & closed subtrees untouched) and updates the warm index's `activeWindowNodeId` in place. Like the command-focus path it does **not** save (window focus is volatile / re-derived on load), so there is no durable save-shape change. One convergence guard (added after an independent code review): `focusRuntimeWindowInPlace` clears only the single index-tracked active window, so if the outline has drifted to **>1 active window** the flip could leave a stale one active. The full reconcile heals that by rewriting every window's active flag, so the narrow path falls back whenever the state is not already in a clean ≤1-active-window shape — restoring exact parity with the old path. (Cosmetic/volatile, and the multi-active state is itself invariant-violating and aggressively self-healed elsewhere, but the fallback keeps the narrow path no worse than the reconcile it replaces. Covered by a teeth-verified test.)

Measured on a **realistic** synthetic session (24,957 outline nodes, 99.8% closed; 8 live windows × 6 live tabs = 48 live tabs; native focus switch between two live windows; same seeded state, optimization toggled): global `tabs.query` calls **1 → 0**; controller work **~262ms → ~27ms** (~10×); broadcast unchanged (one `activeStateUpdated`, correct window flags). The two costs the narrow path removes are independent: `tabs.query({})` is `O(live tabs)` (the environment-bound ~355-655ms call on the user's machine; here count 1 → 0), and the clone + `reconcileWithWindows` + material diff is `O(total outline nodes)` (~25k, dominated by closed history) — replaced by an `O(windows)` `windows.getAll(populate:false)` shell read + an `O(1)` in-place flip. (An earlier note cited "240ms / 25k tabs"; that harness wrongly modeled the 25k as *live* tabs — corrected here. The window the focus leaves/targets does not affect the cost: the old path scans every window and clones the whole tree regardless.)

- **Asymptotics:** `Current Asymptotics Audit` updated — new "Native window focus-gain" row (`O(w)` shell read + `O(1)` in-place flip; full `O(w log w + n)` reconcile only on the ambiguous fallback), and the "Full runtime reconciliation fallback" row's main-win note now lists focus-gain among the prevented narrow events. This is the focus-GAIN counterpart to the 2026-06-14 focus-LOSS (`WINDOW_ID_NONE`) early-return.
- **Tests:** 3 new deterministic `controller.test.ts` tests — in-place flip with no `tabs.query`; stale-event correction (event names window A, truth is window B focused → B wins); unknown-focused-window fallback to a full reconcile that attaches it. The existing `concurrent-focused-window-then-group` generated traces cover the stale-focus-during-command race (they were the ones that caught a naive event-trusting first cut).
- **Guards:** full vitest 806 pass / 2 skip; `trace-hunt:runtime` 911 traces, 0 findings (incl. `focus`/`stale-event`/`race`/`partial-snapshot`/`scope-order` tags); `test:soak` 474 pass; `node scripts/perf-runtime-guard.mjs --hard-only` PASS (9 scenarios — native focus-gain is not a guarded synthetic scenario; all counters unchanged); `perf:sidebar-projection-guard` PASS; storage-fault lane PASS (run despite no durable save-shape change, since the path drops a volatile-state save); lint + `typecheck:test` + `oracle:build` + `build` clean. No budget changes. The chronic report-only `command-group-live-leaf firstBroadcastMs` timing note is unrelated (command path, untouched by this change).
...
- **No perf benefit** — the per-write ceiling was already lifted by #16–#18 (the manifest is a ~KB pointer). This is correctness-hardening + finishing the "empty storage.local" goal: after Step 4 a normal external save writes **only** to IndexedDB (manifest no longer hits storage.local), which now holds just the boot snapshot + incident log + prefs (+ the TTL-expiring migration backup).
- **Atomic commit.** `commitV4SnapshotItems` (external) writes shards + manifest in one `shardStore.set`; history rides after (history-behind on crash, as before); the boot snapshot stays on storage.local. `rollbackV4SnapshotItems` mirrors the new split.
- **Load.** `loadStateV4(api, shardStore, manifestStore)` gained a `manifestStore` param (default storage.local — the default path is byte-identical). `loadV4WithShardMigration` now tries the manifest in the shard store first (P4 steady state), then storage.local + IDB shards (P3 = current post-#17/#18 users), then the pre-#17 both-on-storage.local path.
- **Migration (verify-before-delete, gated).** `migrateManifestToStore` copies both manifest slots storage.local → IndexedDB (double-buffer intact), confirms a fresh r0 load from IndexedDB (manifest + shards) materially equals the tree, then deletes the storage.local manifest. The pre-#17 path moves the manifest into IndexedDB and only deletes the storage.local shards **after** that verify passes, so storage.local is never left with a manifest whose shards (or shards whose manifest) it no longer has — it stays a complete fallback until IndexedDB is proven complete; a failed manifest move keeps the storage.local fallback and retries next startup.
- **Bootstrap-over guard hardened.** The guard that refuses to bootstrap-from-windows over an existing store now checks the IndexedDB manifest (not just storage.local) and **fails safe when the shard store is unreadable** — a transiently-unavailable IndexedDB is never mistaken for a fresh profile (which would orphan the real tree and let a later save overwrite it).
- **Adversarial review (durability):** no BLOCKER; core (atomic save, default-path byte-identical, verify-before-delete) confirmed sound. M2 (cleanup blanket-deletes storage.local shards) is moot — `copyStateV4Shards` copies both slots, so the cleanup only removes shards already in IndexedDB. M1 (storage.local left with a manifest but no shards on a crash mid-migration) fixed by gating the storage.local shard-delete on a verified-complete IndexedDB (`migrateManifestToStore` now returns success). Recovery ladder r0/r1/r2 kept.
- **Gated:** with no external shard store (tests/guard) the save is one storage.local set, byte-identical — runtime-guard counters unchanged.
- **Tests:** atomic-save (manifest in IndexedDB, not storage.local; reloads from IndexedDB), P3 manifest migration on boot (the current-user upgrade path), manifest-migration-failure keeps a complete storage.local fallback, bootstrap-over guard refuses when IndexedDB is unreadable, and the updated shard-migration test (manifest also moves to IndexedDB).
- **External-path soak (closes the coverage gap):** a generated soak (`pnpm test:soak`, soak-scaled to 8 seeds × 120 steps) drives the real coordinator through grow/churn/shrink mutations + reload restarts against an IndexedDB store, asserting after every save that the store round-trips the exact tree (atomic shards+manifest commit + manifest-in-IndexedDB load) and that closed nodes survive each reload — the external coordinator path the storage-fault lane + storage-v4 property test (default storage.local path) do not exercise.
- **Validation:** scaled hunts+soaks on the branch — storage-v4 property soak 400 seeds × 1000 steps (data integrity held), full generated soak clean, runtime-trace hunt's 5 findings all verified pre-existing on `main` (runtime-domain, orthogonal to storage).
- **Guards:** full vitest 777 (+5), typecheck (src + test) + build clean, `perf-runtime-guard --hard-only` PASS (9, counters unchanged), `perf:sidebar-projection-guard` PASS (2), storage-fault lane PASS (corpus + crash soak).

### 2026-06-15: DATA-LOSS FIX — orphan shard sweep vs split-save race (regression from #17)

In-browser dogfooding of #17 (shards → IndexedDB) corrupted the tree: an exported tree went from 24,383 nodes to 23,723 (~660 lost), with hundreds of tabs re-rooted to the top level and root order scrambled. The incident log showed `v4LoadRecovery recovery:"r2"` (full salvage, `parentMismatchCount:484`) — the load couldn't read shards the manifest referenced, so salvage re-rooted the orphans and dropped what it couldn't reconnect.

Root cause: #17 made the save a **split across substrates** — `shardStore.set(shards)` (IndexedDB, committed) then `storage.local.set(manifest)`. The `outline:v4:nodes:*` orphan sweep (`scheduleOrphanShardSweep`, fire-and-forget 8 s after startup, **not serialized with saves**) deletes shard keys "no stored manifest references." During the window between the shard write and the manifest commit (seconds, since `storage.local.set` is slow on the affected profile), the just-written shards are in IndexedDB but the new manifest is not yet stored, so the sweep classifies them as orphans and deletes them — then the manifest commits referencing the now-deleted shards. The next load fails → r2 salvage → re-rooting + node loss. On the legacy storage.local path the save was one atomic set, so the sweep could never see new-shards-without-new-manifest; the split broke that invariant.

Fix: skip the orphan sweep when the shard store is external (IndexedDB). The per-save GC (`removeKeysAfterCommit`, serialized within the save and scoped to the well-defined two-generations-back keys) plus the `previousV4Snapshot` seeding keep generations bounded without a scanning sweep, so the sweep — a storage.local-whole-store-read-cost backstop — is unnecessary there. (A save-serialized sweep could be re-added later if orphan accumulation is ever observed.)

- **Recovery:** no code can un-corrupt an already-salvaged store; restore from a clean portable-tree export (or the `outline:v4:migrationBackup` copy) onto a fixed build. Do NOT revert #17 — the shards already live in IndexedDB and the storage.local copies are deleted, so reverting would read from the empty storage.local and salvage/bootstrap, losing more.
- **Test:** new controller regression — with an external shard store, planting an unreferenced shard and advancing past the 8 s sweep delay leaves it intact and records no `orphanShardSweep` (teeth-verified: without the guard the sweep deletes it).
- **Guards:** vitest 768 (+1), typecheck (src + test) + build clean, `perf-runtime-guard --hard-only` PASS (9), storage-fault lane PASS (corpus + crash soak).

### 2026-06-15: Storage write-cost fix step 3b — undo history → IndexedDB (finish emptying storage.local)

After #17 (shards → IndexedDB), an in-browser profile showed storage.local down 35 MB → 12 MB but `state.save` still spiking to ~1.2 s on some saves: the remaining storage.local bulk is the **2.9 MB undo history** (rewritten on history-changing saves) plus the one-time 8.8 MB migration backup (TTL-reclaimed). This moves the undo history (`outlineHistory`) off storage.local onto the same IndexedDB bulk store as the shards/journal, so a history-changing save no longer rewrites a multi-MB storage.local area. After this, storage.local holds only the manifest, boot snapshot, incident log, prefs (and the temporary backup) — ~KB once the backup expires, so *every* storage.local write is ~ms.

- **Save ordering (durability):** `commitV4SnapshotItems` writes shards → shard store, manifest → storage.local (the shadow-paging commit), then history → shard store **AFTER** the manifest. So a crash leaves history *behind* the committed tree, never ahead — and history-behind is exactly what the journal's `historyEntryId` recovery already rebuilds, whereas history-ahead (an undo entry for a tree change the manifest doesn't reflect) would be the dangerous case and is avoided. History is losable (the undo stack, not the tree), so the bar is "never corrupt the tree / never apply a stale undo," which the ordering meets.
- **Migration + load:** `migrateHistoryToStore` (startup, external only) copies the storage.local history to the store then deletes the local copy (idempotent; failure leaves storage.local authoritative). `loadHistory(api, limit, store)` reads the store, falls back to storage.local, and **re-checks the store on a double-miss** so a concurrent migration delete (the lazy `ensureHistory` warmup is not serialized behind the migration) can't make it read an empty stack and overwrite the real one.
- **Gated:** with no external store (tests/guard) history stays in the combined storage.local set — byte-identical, runtime-guard counters unchanged.
- **Review:** adversarial subagent pass — **no BLOCKER/MAJOR** (tree safe across save-order, migration, fallback, gating, atomicity-regression; history never ahead). Found one MINOR (a first-upgrade-boot warmup-vs-migration race that could drop the undo stack); fixed by the loadHistory store re-check.
- **Tests:** controller (history-tracked command → history in IndexedDB not storage.local, loadHistory returns the real undo entry; existing storage.local history migrates to IndexedDB on boot); storage (loadHistory re-checks the store after a double-miss; storage.local fallback).
- **`Current Asymptotics Audit` (Persistence row):** history writes move from the O(total storage.local) area to an O(payload) IndexedDB put; combined with #16/#17, storage.local writes are now ~KB.
- **Guards:** full vitest 772 (+4, rebased onto #19), typecheck (src + test) + build clean, `perf-runtime-guard --hard-only` PASS (9, counters unchanged), `perf:sidebar-projection-guard` PASS (2), storage-fault lane PASS (corpus + crash soak).

### 2026-06-15: Storage write-cost fix step 3 — node shards → IndexedDB (the ceiling-lifter)

Step 3 of the IndexedDB migration (docs/storage-rearchitecture/04-STORAGE-WRITE-COST.md §6). The post-#16 profile (`tabs-outliner-profile-2026-06-15.json`) confirmed the journal is now ms on IndexedDB (28/32 ms, the 565 ms outlier was queue-wait behind a reconciliation) but `background.state.save` (the v4 shard compaction) was still **131–427 ms even on a calm-I/O capture** (census probe 14 ms) — because a save rewrites whole dirty shards into the O(total-store) `storage.local` area. This moves the bulk node shards to the extension-owned IndexedDB store (the same store as the journal — distinct key prefixes `outline:v4:nodes:` vs `outline:v4:journal:`), so a shard write becomes O(payload). The manifest (a small double-buffered pointer) stays on `storage.local` as the shadow-paging commit point — and because the bulk has left, *every* remaining `storage.local` write (manifest, history, boot snapshot) gets fast too. The v4 journal/snapshot algorithm is unchanged.

- **Gated on an external shard store.** `shardStore` is injected only in production; absent (tests/guard) the save path is **byte-identical to today** (one combined `storage.local` set, no migration, no deletion) — the runtime-guard counters are unchanged (`saves=1`, `storageSetCalls=2`), confirming the default path is untouched. `storage-v4.ts` (`loadStateV4`/`loadStateV4FromManifest`/`salvageStateV4`/`sweepOrphanedV4Shards`/`hasAnyStateV4Keys`) takes the `shardStore`; the coordinator splits the save (`commitV4SnapshotItems`: shards → shardStore, then manifest+history → storage.local) and routes shard GC to it.
- **Migration (verify-before-delete, no data loss):** `loadV4WithShardMigration` (folded into the startup load, so the dominant read happens **once**) copies the live shards storage.local → IndexedDB, confirms a fresh `loadStateV4(shardStore)` is r0 **and** `statesMateriallyEqual`, writes a portable-tree backup (reusing the migration-backup key + its TTL), and only **then** deletes the storage.local shards. Any failure (verify mismatch, torn copy, IndexedDB unavailable) leaves storage.local authoritative and retries next run.
- **Resilience (adversarial-review findings, fixed):** (M1) the migration's detection no longer double-reads the tree — the load is folded in and returns the authoritative result. (M2) every IndexedDB op is bounded by a 30 s timeout so a wedged transaction can't hang startup; on timeout it falls back to the storage.local shards. A shard-store error during load falls back to storage.local; a v4 manifest present with no loadable shards **refuses to bootstrap-over** (throws `bootstrapSkippedShardStoreUnreachable`) so a transiently-unavailable IndexedDB never overwrites the real tree. The migration-backup meta is read after the load to avoid a first-run TTL-expiry race.
- **Review:** two adversarial subagent passes (the data migration is where the shards ARE the user's data). No data-loss BLOCKER — verify-before-delete holds across every crash interleaving; all five attack surfaces (migration loss, post-migration IDB-unavailable, save-split atomicity, mixed-substrate, gating) confirmed safe. The two MAJORs (double-read, hang) and two MINORs (torn-IDB test, R1 best-effort) are addressed.
- **Tests:** `storage-v4-indexeddb.test.ts` (load shards-from-IDB + manifest-from-local, `copyStateV4Shards`, `deleteAllStateV4ShardKeys`, sweep on IDB) and controller tests (migrates on boot + reloads exact tree + no re-migrate; **torn IDB copy aborts, storage.local retained**; shard-store-unavailable boots from storage.local).
- **`Current Asymptotics Audit` (Persistence row):** shard writes move from O(total `storage.local` store) to O(payload) IndexedDB; `storage.local` drops from ~13 MB to ~KB so its remaining writes get fast too. The startup *load* still reads all shards (O(n)) but from IndexedDB.
- **Guards:** full vitest 767 (+7), typecheck (src + test) + build clean, `perf-runtime-guard --hard-only` PASS (9, counters unchanged), `perf:sidebar-projection-guard` PASS (2), **storage-fault lane** PASS (fault corpus + crash soak — the storage.local fault behavior is unchanged on the default path). Per the plan this lands behind a Step-2 dogfood window; the PR is ready but should merge after Step 2 has soaked.
