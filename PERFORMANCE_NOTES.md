...
Measured (2-window synthetic, 25k tabs in the unfocused source window, native focus switch): global `tabs.query` calls 1 → 0; controller work 240ms → 3ms; broadcast unchanged (one `activeStateUpdated`, correct window flags). The cheap `windows.getAll(populate:false)` shell read replaces the all-tabs query.
