...
- Profile results after `pnpm run build`:
  ...
  - `pnpm exec playwright test tests/playwright/sidebar-first-paint.spec.ts --reporter=list`: first rows at ~68ms focused / ~77ms in the full Playwright run when the active tab is in the snapshot, one initial snapshot request, no full hydration before rows are visible. A second test verifies the sidebar stays hidden rather than revealing a top slice when the snapshot misses the active tab.
