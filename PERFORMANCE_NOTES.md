...
### 2026-06-15: Restore-from-export (replace) recovery command

A disaster-recovery command for putting a clean portable-tree export back after a corruption incident (the #17 orphan-sweep data loss below). The existing `importTree` *appends* the file under a new "Imported tabs" group; `restoreTree` *replaces* the whole outline with the export's roots at top level and reloads the extension so the **tested** startup path re-loads the restored tree and reconciles the live browser session.

- **Model:** `restorePortableTree(payload, clock)` builds a fresh `OutlineState` whose top-level roots ARE the export's roots (no import-group wrapper, every node `status:"closed"`), and **throws on an empty export** so a corrupt/empty file can never wipe the outline to nothing. Distinct from `appendPortableTree` (merge-append).
- **Background (`restoreTreeFromBackground`, serialized in `enqueueMutation`):** validate first (malformed → nothing changes), set `state = base` + reset undo history to empty, force a **full-compaction save** (no candidate ids) that stamps `journalSeqIncluded` at the journal head and prunes — so no stale mutation replays on top of the restored tree after reload — clear the stale boot snapshot, then `api.runtime.reload()` (injected via the runtime API; a fake records it in tests). After reload, currently-open windows reconcile back as live nodes alongside the restored closed snapshots (the user dedupes those by hand — accepted).
- **Adversarial review (data-replacing, post-incident) found two real defects, both fixed:**
  - **BLOCKER:** the handler began with `await ensureState()`, which *throws* the bootstrap-over guard in the exact corruption it targets (a v4 manifest whose shards are gone) — blocking recovery when it's needed most. Fixed: `ensureState().catch(() => undefined)` (the journal is still initialized during the refused load — `createAndInitJournal` runs before the guard — so the full-save seq-stamp + prune still hold). Test: seed a manifest with no shards, confirm `ensureState` rejects, confirm restore still commits + reloads + loads clean.
  - **MAJOR:** on save failure the handler claimed "leave prior state in place" but had already mutated in-memory state, and the failed save's armed backoff retry would silently persist the replacement later **without a reload/reconcile**. Fixed: new coordinator `discardPendingSaves()` cancels the armed retry + drops the queued save, and the failure path rolls in-memory state/history back to the pre-restore session (the on-disk store is intact via shadow paging). Test: shard `set` fails once → `{ok:false}`, no reload, in-memory rolled back, and advancing past the backoff persists nothing (teeth: `set` works again, so a surviving retry would have landed the replacement).
  - Verified-safe by the review: stale-journal replay (full-save seq-stamp + prune), split-save durability before reload (IDB `set` resolves on tx complete; `flushPendingSaves` awaits the whole commit), orphan sweep (#19 external-store guard), reconcile not cannibalizing the restored closed tree (imported ids don't match runtime ids), empty/duplicate/Chrome-format payloads, concurrency (restore is high-priority; runtime refreshes are low-priority + serialized).
- **UI:** "Restore from file (replace)" in the sidebar overflow menu, behind a `window.confirm` warning (replaces everything, reloads, live windows may duplicate). Stays enabled even when the tree failed to load (it is a recovery tool).
- **Guards:** full vitest 774 (+6), typecheck (src + test) + build clean, `perf-runtime-guard --hard-only` PASS (9, counters unchanged — restore is a cold, user-triggered path), `perf:sidebar-projection-guard` PASS (2), storage-fault lane PASS (fault corpus + crash soak).
