...
| Persistence | interaction path is deferred/coalesced; eventual v3 saves are `O(changed shards/pages)` for narrow changes and `O(n)` for full replacement, migration, or broad edits. The 256-row boot snapshot is no longer rebuilt per save (P0.5, 2026-06-10): it is written to its own key on a 10s debounce off the interaction path | Same lower bound unless storage layout changes | Keep perceived latency separate from durability. Do not force-drain unrelated broad saves on narrow interaction paths. Boot snapshot staleness is bounded by the debounce and superseded by full hydration. |
