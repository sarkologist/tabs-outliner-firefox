...
Each journal-append row also carries a domain-level description (`outline-change-summary.ts`: "Deleted 'Work' (window) (+12 descendants)", "Moved 'Gmail' → 'Personal'", etc.) so an unexpected change is obvious by name, not just by count. It is computed in `journalDeltaItem` from the **already-built** delta plus the before/after states (O(delta), no extra tree walk) and carried on an in-memory-only `changeText` field that the journal never persists.
