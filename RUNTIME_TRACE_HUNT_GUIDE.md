...
Opt-in order assertions:

- `runtimeOrder` means live outline preorder must equal the fake browser tab-strip order. Use it for flat/browser-authored order traces where outline hierarchy is intentionally the tab-strip linearization.
- `runtimeScopeOrder` means live ledger window scope order must equal the fake browser tab-strip order. Accepted window shape fact order is also checked when the fact has current live tab membership; partial or missing-window facts do not pretend to be current order authority.
- Prefer `runtimeScopeOrder` for command-owned undo/redo, restore, recovered delete, and history replay traces where outline hierarchy may be logical or nested while the ledger scope is the current browser-order truth.
- Do not make `runtimeOrder` skip risky windows silently. If outline preorder is not expected to equal browser order, remove `runtimeOrder` from that trace and add `runtimeScopeOrder` instead.
...
- A fix that changes **save timing or save shape** (journal append points, compaction triggers, deferral schedules, checkpoint conditions) must also pass the storage-fault lane (`autoresearch/CORRECTNESS_GUARDS.md` storage-faults lane: torn-write/failed-set/crash-restart tests plus the soak-scaled crash/restart property run) before findings are marked fixed — this class of change historically traded crash-loss windows for latency without a gate noticing.

## Incident Log Triage

The durable incident log (`tabsOutlinerIncidentLog:v1`, viewable on the options page) records anomalies only: `saveFlushAnomaly`, `stateSaveFailed`, `closedSubtreeGuardRestore`, `storageLoadStructureRepair`, `v3LoadSalvaged`, `v4LoadRecovery`, `v4MigrationFailed`, `staleV2FallbackUsed`, `bootstrapSkippedStoredDataPresent`, `journalReplay` (and routine startup/lifecycle/backup markers). Any non-routine incident observed during dogfooding is treated like a failing hunt trace: freeze the evidence (export the incident log and a `tabsOutlinerProfile` trace), reduce it to a deterministic trace, and file it in `RUNTIME_TRACE_BUGS.md` with the incident name as part of the signature. `journalReplay` at startup is routine only after a crash; repeated `journalReplay` on every startup means compactions are failing and should be triaged via `stateSaveFailed`/`compaction` evidence.
...
| Process death / storage fault | crash before journal append, after append before compaction, mid-compaction set, after set before GC, mid journal-slot write, failed/torn `storage.local.set`, migration interruption | the fault-injecting storage mock (`src/test/faulty-storage.test-support.ts`: `failNextSet`, `tearNextSet`, hung sets) decides what physically landed; verified load (R0–R4 ladder) plus journal replay decide what survives | invariants I-1 (acked mutations survive restart), I-2 (no silent stale load), I-3 (consistency verifiable from storage alone), I-5 (crash-safe compaction) | covered by `storage-v4.test.ts` (ladder + generated crash/restart property test, soak-scaled via `pnpm test:soak`), `outline-journal.test.ts`, and controller I-1/crash-mid-compaction/migration-failure tests; the storage-faults autoresearch lane gates save-timing experiments | mutate along this axis when adding hunts for save-path changes: combine a lifecycle command or native event with a kill at each crash point and assert the restart state, instead of assuming the in-memory model survived |
...
This hunt started from the `251` regression / `856` discovery baseline after truth-cache and runtime side-effect oracles were added. It targeted oracle gaps, not a behavior fix pass. New failing traces were frozen as RT-222 through RT-242, then fixed and promoted by the runtime truth cleanup pass.
...
- New signatures: none.
- Coverage movement: sampled-clean for the new Rung 1 oracle scope-mix traces; at hunt time, open RT-222 through RT-242 still dominated full-corpus failure output. They are now fixed/promoted.
...
- New signatures: none.
- Coverage movement: sampled-clean for temporal oracle heat across command rejection/history/session/query/stale-echo combinations; at hunt time, RT-222 through RT-242 remained the only discovery failures. They are now fixed/promoted.
- Dedupe/result: second clean active block after the Block 1 reset; one more clean block is required for the stop condition.
- Block: 4
- Active effort: about five minutes of final escape-cell selection, trace edits, explicit replay, duplicate-failure corpus review, and evidence-log update; corpus wait time excluded.
- Rung: final escape after temporal heat
- Axes changed: pure browser-event side-effect silence, short tab close persistence, undo/redo side-effect replay, focus-rejection metadata/order.
- Temporal boundaries crossed: browser update/focus/window-state before reordered refresh and restart; final tab close before save/reload persistence; history replay around stale query evidence; rejecting focus update before reordered current evidence.
- New/changed trace ids: `oc-b4-browser-events-side-effect-escape`, `oc-b4-short-tab-close-persistence`, `oc-b4-history-side-effect-replay`, `oc-b4-focus-reject-metadata-order`
- Explicit replay result: four-trace replay passed.
- Discovery runner result: `872` traces, `304` vitest processes, `21` duplicate failures, `0` new findings, completed corpus.
- New signatures: none.
- Coverage movement: sampled-clean for final oracle escape probes; no new failure class beyond RT-222 through RT-242, which are now fixed/promoted.
- Dedupe/result: third clean active block after the Block 1 reset; runtime oracle hunt stop condition reached.

## Runtime Scope-Order Hunt 2026-05-31

This hunt started from the `276` regression / `856` discovery baseline after the runtime order oracle upgrade. It targets ledger/window-scope order authority in command-owned history, restore, and recovered delete flows where outline preorder is not necessarily browser tab-strip order. Production reconciliation is not changed during the hunt.

- Block: 1
- Active effort: about five minutes of active scope-order target selection, trace design, DSL/action review, trace edits, explicit replay, corpus review, and evidence-log update; corpus wait time excluded.
- Rung: 0
- Axes changed: recovered delete after command nesting, restored promoted child replay, command-created destination undo/redo, browser-authored tab-strip moves, stale old-window echoes, reordered queries, `runtimeScopeOrder` assertions.
- Temporal boundaries crossed: command nesting before delete side-effect recovery; restore-create rejection before browser-created child and history replay; command-created destination before browser-authored insertion and undo/redo; stale old-window echoes after current scope order changed.
- New/changed trace ids: `so-b1-recovered-delete-nested-scope-order`, `so-b1-restore-replay-promoted-child-scope-order`, `so-b1-command-destination-undo-redo-scope-order`
- Explicit replay result: three-trace replay passed.
- Discovery runner result: `859` traces, `43` vitest processes, `0` failures, completed corpus.
- New signatures: none.
- Coverage movement: sampled-clean for basic `runtimeScopeOrder` authority across recovered delete, restored/browser-created child replay, and command-created destination undo/redo. This does not make the cell covered; snapshot confidence and temporal-heat variants remain for Blocks 2 and 3.
- Dedupe/result: first clean active block; bug yield none; raise to Rung 1.
- Block: 2
- Active effort: about five minutes of active Rung 1 axis selection, trace design around snapshot confidence and event-source changes, trace edits, explicit replay, corpus review, and evidence-log update; corpus wait time excluded.
- Rung: 1
- Axes changed: missing-window query evidence, real browser-authored same-window reorder after command history, session-only native close evidence, stale old-window echo after tombstone, complete refresh recovery, `runtimeScopeOrder` assertions.
- Temporal boundaries crossed: command-created destination before missing-window/reordered refresh; command history replay before real browser reorder; undo/redo before session-only tab disappearance and stale old-window evidence.
- New/changed trace ids: `so-b2-partial-window-scope-order`, `so-b2-real-browser-reorder-scope-order`, `so-b2-session-close-history-scope-order`
- Explicit replay result: three-trace replay passed.
- Discovery runner result: `862` traces, `44` vitest processes, `0` failures, completed corpus.
- New signatures: none.
- Coverage movement: sampled-clean movement from basic scope-order authority into snapshot confidence, actual browser reorder acceptance, and session-only close/tombstone evidence. Temporal heat across restart/restore/delete replay remains for Block 3.
- Dedupe/result: second clean active block; bug yield none; raise to Rung 2 temporal heat.
- Block: 3
- Active effort: about five minutes of active temporal-heat design, trace edits, explicit replay, corpus review, final-close persistence scoping, and evidence-log update; corpus wait time excluded.
- Rung: 2
- Axes changed: abrupt restore materialization, recovered delete plus undo/redo replay, mixed saved/restored/browser-created/command-created provenance, partial snapshot before abrupt redo, stale command/browser echoes, short final outliner close persistence, `runtimeScopeOrder` assertions.
- Temporal boundaries crossed: restored materialization across abrupt restart before browser-created cohabitation; recovered delete before undo crash replay and redo; three-provenance cohabitation before missing-window query and abrupt redo; command history before final close persistence.
- New/changed trace ids: `so-b3-abrupt-restore-delete-redo-scope-order`, `so-b3-three-provenance-scope-order`, `so-b3-short-final-close-persistence-scope-order`
- Explicit replay result: three-trace replay passed.
- Discovery runner result: `865` traces, `44` vitest processes, `0` failures, completed corpus.
- New signatures: none.
