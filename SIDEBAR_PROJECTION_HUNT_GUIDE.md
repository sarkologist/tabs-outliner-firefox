...
Current projection guard baseline after 2026-05-27 sparse boundary work:
...
- Projection discovery corpus has 161 `psh-*` scenarios covering rejected/delayed/out-of-order slices, sparse full-state hydration, compact patch/refill behavior, covered and missing-coverage action truth, remote search clear/show-in-tree slices, owner replacement, target-node freshness, stale search/scroll timing, edit/history command ordering, multi-sidebar owner independence, covered sparse drag/drop before full hydration, sparse-local hover/rename/keyboard/drag/drop/focus/close/delete interactions, cut/paste recovery, restore/delete shells, command-to-search/target replacement, missing-coverage drag recovery after covered refill, restored-tab root drops, rename-to-show-in-tree replacement, and cross-sidebar boundary drag plus target reveal fanout.
- Fixed findings are preserved as regression coverage in the `psh-*` corpus: `PT-001` through `PT-031`; the suspected `PT-032` rename/search issue was retracted and its scenarios now pass as stale-refill-before-search coverage. Do not mutate those scenarios directly during discovery; clone variants with new neutral IDs.
...
- keyboard shortcuts crossing search/show-in-tree replacement while rename, drag, or cut state is active; pointer rename-to-search/target replacement is now sampled-clean;
