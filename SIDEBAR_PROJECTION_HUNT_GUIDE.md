...
Current projection guard baseline after the 2026-05-27 collapsed-boundary hunt:
...
- Fixed findings are preserved as regression coverage in the `psh-*` corpus: `PT-001` through `PT-031` and `PT-033` through `PT-035`; the suspected `PT-032` rename/search issue was retracted and its scenarios now pass as stale-refill-before-search coverage. `PT-036` and `PT-037` are frozen expected-failing collapsed-boundary repros pending the next fix pass. Do not mutate those scenarios directly during discovery; clone variants with new neutral IDs.
