...
Current projection guard baseline after the 2026-05-27 restore/keyboard/temporal hunt:
...
- The 2026-05-27 restore/keyboard/temporal hunt added 15 scenarios across delayed closed-restore scopes, restored/delete shells, keyboard cut/paste/undo/redo while query or show-in-tree ownership is changing, and two-sidebar restore/search/target independence. It found `PT-033` and `PT-034`, then stopped after three clean active mutation blocks with no further distinct projection signatures.

Next sparse cells:

- drag/drop across collapsed ancestors, unloaded children, and non-visible sibling boundaries where the sidebar must decide whether local placement is covered; basic covered/missing-coverage row/root drops are now sampled-clean;
- restore/reopen workflows for closed rows while coverage is partial or recently refilled, especially after the `PT-033` fix closes stale restore-scope prompts;
- keyboard shortcuts crossing search/show-in-tree replacement while rename, drag, or cut state is active, especially after the `PT-034` fix closes target-owner loss under history/title patches;
