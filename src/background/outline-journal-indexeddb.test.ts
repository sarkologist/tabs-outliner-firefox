...
import { JOURNAL_META_KEY, JOURNAL_SLOT_PREFIX, createOutlineJournal, migrateJournalStore } from "./outline-journal.js";
...
describe("outline journal on IndexedDB (the production substrate)", () => {
  it("round-trips appended entries across a reopen", async () => {
    ...
    const reopened = createOutlineJournal(indexedDbKvStore("journal-idb-roundtrip", "kv"), { epoch: 2 });
  ...
  it("migrateJournalStore moves a storage.local journal onto IndexedDB, preserves entries+epoch, drops the source", async () => {
    ...
    await seeded.init();
    await seeded.append([{ kind: "command", label: "close", delta: { deletedNodeIds: ["tab:9"] } }]);
    expect(local.snapshot()[JOURNAL_META_KEY]).toBeDefined();
    expect(Object.keys(local.snapshot()).some((key) => key.startsWith(JOURNAL_SLOT_PREFIX))).toBe(true);
    ...
    expect(Object.keys(local.snapshot()).some((key) => key.startsWith(JOURNAL_SLOT_PREFIX))).toBe(false);
    ...
    const idbMeta = (await idb.get(JOURNAL_META_KEY))[JOURNAL_META_KEY] as { epoch?: number } | undefined;
  ...
  it("migrateJournalStore leaves the source intact when the destination write fails", async () => {
    ...
    await expect(migrateJournalStore(localStore, failingDestination)).rejects.toThrow("indexedDB write failed");
  ...
  it("migrateJournalStore does not clobber an already-migrated destination", async () => {
    ...
    await stale.init();
    await stale.append([{ kind: "command", label: "rename", delta: { updatedNodes: [makeNode("tab:old")] } }]);

    const idb = indexedDbKvStore("journal-idb-authoritative", "kv");
    const current = createOutlineJournal(idb, { epoch: 9, now: () => 2000 });
    await current.init();
    await current.append([{ kind: "command", label: "rename", delta: { updatedNodes: [makeNode("tab:new")] } }]);
