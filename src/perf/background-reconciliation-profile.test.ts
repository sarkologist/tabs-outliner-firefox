...
describe("background reconciliation autoresearch profile", () => {
  ...
  it("requires the configured primary-median improvement", () => {
    ...
    expect(discard.guardFailures).toContain(
      "primary median must improve by at least 50ms from baseline"
    );
  });

  it("compares scenario guard metrics against a baseline summary", () => {
    const baselineSummary = summarizeBackgroundReconciliationRuns(candidateResults(), {
      runs: 2,
      tabs
    });
    ...
    expect(current.guardFailures).toContain(
      "move-leaf saveFlushMs must not increase versus baseline"
    );
    expect(current.guardFailures).toContain(
      "move-leaf storageSetCalls must not increase versus baseline"
    );
    expect(current.guardFailures).toContain(
      "move-leaf stateSaves must not increase versus baseline"
    );
    expect(current.guardFailures).toContain(
      "move-leaf eventEchoMs must stay within 25ms of baseline"
    );
    expect(current.guardFailures).toContain(
      "move-leaf runtime.getWindows count must not increase versus baseline"
    );
    expect(current.guardFailures).toContain(
      "move-leaf runtime.getWindows max must not increase versus baseline"
    );
  });
  ...
  it("guards command relocation echo absorption metrics", () => {
    ...
    expect(summary.guardFailures).toContain(
      "command-relocation-echo must absorb native echoes without runtime.getWindows"
    );
    expect(summary.guardFailures).toContain(
      "command-relocation-echo must not add a second state save for native echoes"
    );
    expect(summary.guardFailures).toContain(
      "command-relocation-echo must not add storage writes for native echoes"
    );
    expect(summary.guardFailures).toContain(
      "command-relocation-echo native echo flush must stay below 25ms"
    );
  });
  ...
  it("guards existing command-window relocation echo absorption metrics", () => {
    ...
    expect(summary.guardFailures).toContain(
      "command-existing-window-relocation-echo must absorb native echoes without runtime.getWindows"
    );
    expect(summary.guardFailures).toContain(
      "command-existing-window-relocation-echo must not add a second state save for native echoes"
    );
    expect(summary.guardFailures).toContain(
      "command-existing-window-relocation-echo must not add storage writes for native echoes"
    );
    expect(summary.guardFailures).toContain(
      "command-existing-window-relocation-echo native echo flush must stay below 25ms"
    );
  });
  ...
  it("guards structural save pressure latency metrics", () => {
    ...
    expect(summary.guardFailures).toContain(
      "structural-save-pressure must not start V3 state saves before command ack"
    );
    expect(summary.guardFailures).toContain(
      "structural-save-pressure follow-up command must not wait for deferred state save"
    );
    expect(summary.guardFailures).toContain(
      "structural-save-pressure must not add runtime.getWindows"
    );
    expect(summary.guardFailures).toContain(
      "structural-save-pressure must coalesce to one eventual state save"
    );
    expect(summary.guardFailures).toContain(
      "structural-save-pressure must not broadcast full stateUpdated messages"
    );
  });
  ...
  it("formats TSV rows for ignored autoresearch results", () => {
    ...
    expect(row).toContain(
      "\t570\t50\t520\tmove-top-level-live-leaf\t510\t520\t210\t310\t510\t80\t10\t1\t12\tcandidate-keep\tfixture"
    );
  });
  ...
  it("summarizes exported tabsOutlinerProfile background bottlenecks", () => {
      ...
      snapshot: {
        background: {
          entries: [
            ...
            {
              name: "background.mutation.run",
              durationMs: 30,
              detail: { reason: "refreshFromRuntime" }
            },
    ...
    expect(analysis.topTotals[0]).toMatchObject({
      name: "sidebar.diagnostics.defer",
      totalMs: 1000
    });
  });
});
