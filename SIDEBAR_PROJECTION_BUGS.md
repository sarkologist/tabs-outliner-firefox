...
- Distinct findings recorded: 38
- Status: `PT-001` through `PT-031`, `PT-033` through `PT-038`, and `PT-039` are fixed. The sparse action follow-up hunt suspected `PT-032` around rename/search replacement, but the follow-up fix pass retracted it as a harness-ordering false positive and kept the corrected scenarios as required-passing coverage. No open projection findings remain. `PT-039` was a *regression* (not a fresh hunt finding): the frozen scenario `psh-two-sidebars-restored-delete-and-search-stay-independent` was added green at `b46d619` and silently went red when `a5bb527` landed; CI never saw it because `playwright.ci.config.ts` ignored the whole hunt suite (now fixed — the deterministic suite runs in CI).
...
- `PT-039`: the optimistic-delete echo guard (`isAlreadyAppliedDeletePatch`, added in `a5bb527`) treated a `treeStructureUpdated` delete patch as an already-applied echo whenever every deleted node was absent from local state, and short-circuited before decrementing `nodeCount`/`closedCount`. That inference is only sound for a fully-loaded sidebar; a sparse/search projection is missing most nodes because it never loaded them, so a *first-time out-of-view delete* (e.g. a second sidebar in search mode while another deletes a node) was misread as an echo and its authoritative total left stale (`1 match / 1001 items` instead of `999`). The fix additionally requires that this sidebar actually *recorded* deleting those nodes (the `deletedNodeRevisionById` ledger written by `recordDeletedNodeIds`/full-state replacements) before absorbing the patch. Genuine echoes — optimistic delete (full sidebars only), duplicate broadcasts, post-replacement echoes — stay absorbed in both modes; a real out-of-view delete now flows through `applyDeleteTreeStructurePatchToProjection`, which decrements the counts without any remote refill. Restores **I-14** (compact patch application leaves sidebar state equivalent to the next full state for the changed surface — here the total-count chrome).

## Finding Index

- Fixed projection findings: `PT-001`, `PT-002`, `PT-003`, `PT-004`, `PT-005`, `PT-006`, `PT-007`, `PT-008`, `PT-009`, `PT-010`, `PT-011`, `PT-012`, `PT-013`, `PT-014`, `PT-015`, `PT-016`, `PT-017`, `PT-018`, `PT-019`, `PT-020`, `PT-021`, `PT-022`, `PT-023`, `PT-024`, `PT-025`, `PT-026`, `PT-027`, `PT-028`, `PT-029`, `PT-030`, `PT-031`, `PT-033`, `PT-034`, `PT-035`, `PT-036`, `PT-037`, `PT-038`, `PT-039`
...
### PT-039 out-of-view delete leaves a search-mode sidebar's total count stale

- Status: fixed
- Invariant restored: **I-14** (compact patch application leaves sidebar state equivalent to the next full state for the changed surface).
- Regression: introduced by `a5bb527` (`perf(sidebar): instant hover feedback + optimistic closed-node delete`, 2026-06-13). The frozen scenario was added green at `b46d619` (2026-05-27) and flipped red at `a5bb527`; confirmed by checkout (passes at `a5bb527^`, fails at `a5bb527`, identical harness — pure product change). CI did not catch it because `playwright.ci.config.ts` ignored the whole `sidebar-projection-hunt.spec.ts` suite.
- Found by: `psh-two-sidebars-restored-delete-and-search-stay-independent`
- Repro:

```sh
pnpm exec playwright test tests/playwright/sidebar-projection-hunt.spec.ts --grep "psh-two-sidebars-restored-delete-and-search-stay-independent" --reporter=list --workers=1
```

- Expected: two sidebars project the same 1001-item tree. Sidebar A deletes `tab:2`; the background broadcasts a delete patch `["tab:2", "window:20"]` (2 nodes). Sidebar B is in search mode (`"Tab 900"`) and does not have those nodes in its sparse projection. B should keep its search result and drop its total to `1 match / 999 items` with no remote refill.
- Actual before fix: B kept the stale `1 match / 1001 items`. The search row and A's side were correct; only B's total was stale. The `requests`/`stateRequests` assertions still passed (no remote refresh), which pinpoints the early-return echo guard as the only code path that absorbs a delete without touching the count or issuing a request.
- Root cause: in `applyTreeStructureUpdate`, `isAlreadyAppliedDeletePatch(state, deletedNodeIds)` returns true whenever every deleted node is absent from local `state.nodes`. For sparse sidebar B both `tab:2` and `window:20` were absent only because B never loaded them, so the real delete was wrongly absorbed as an echo and `nodeCount` was never decremented.
- Fix: absorb the patch only when `isAlreadyAppliedDeletePatch` holds **and** this sidebar actually recorded deleting every one of those nodes (`deletePatchAlreadyRecordedLocally`, backed by the `deletedNodeRevisionById` ledger). Optimistic delete runs only for fully-loaded non-sparse non-search sidebars, so genuine echoes are always recorded first; a first-time out-of-view delete in a sparse/search sidebar is not, so it now flows through `applyDeleteTreeStructurePatchToProjection` and decrements the total without a remote refill. A duplicate broadcast is recorded by the first application, so it is still absorbed (no double-decrement).
- Verification: focused replay green; full projection hunt corpus 212/212 green (`--workers=1`); `pnpm perf:sidebar-projection-guard` PASS (2 scenarios, 5 runs); `node scripts/perf-runtime-guard.mjs --hard-only` PASS. The suite now runs in CI (`playwright.ci.config.ts` no longer ignores it).
