...
test.describe("sidebar restored single-tab window delete", () => {
  test("does not leave clicked delete actions stuck while the delete patch is pending", async ({
    page
  }) => {
    ...
    await expect(sentSidebarCommands(page)).resolves.toEqual([
      { type: "deleteNode", nodeId: "tab:2" }
    ]);
    expect(issues).toEqual([]);
  });

  test("removes the restored window shell when deleting its only restored tab", async ({
    page
  }) => {
    const issues = collectPageIssues(page);
...
async function loadSidebar(
  ...
  await page.addInitScript(
    ({ state: initialState, emitPatchOnDelete, sparseInitialSnapshot }) => {
      ...
      function treeStructureUpdate(
        ...
        const updatedNodes = Object.keys(next.nodes)
          .filter(
            (nodeId) =>
              JSON.stringify(previous.nodes[nodeId]) !== JSON.stringify(next.nodes[nodeId])
          )
      ...
      function initialTreeSnapshot(outline: {
        ...
        nodes: Record<
          string,
          { id: string; childIds: string[]; collapsed?: boolean; kind: string; active?: boolean }
        >;
      }) {
        const rows = [
          {
